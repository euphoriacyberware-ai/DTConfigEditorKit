#if DEBUG
import Foundation
import DTConfigEditorKit

/// Sample fixture configs available in debug builds for quick testing
/// without pasting from clipboard.
enum DebugFixtures {

    struct Fixture: Identifiable {
        let id: String
        let label: String
        let data: Data
    }

    static let all: [Fixture] = [
        Fixture(id: "basic", label: "Basic Config (1 LoRA)", data: Data(basicConfigJSON.utf8)),
        Fixture(id: "minimal", label: "Minimal Config", data: Data(minimalConfigJSON.utf8)),
        Fixture(id: "controlnet", label: "ControlNet Example", data: Data(controlNetConfigJSON.utf8)),
    ]

    static func load(_ fixture: Fixture) throws -> DrawThingsConfiguration {
        try DrawThingsConfiguration(jsonData: fixture.data)
    }

    // MARK: - Embedded fixture JSON

    private static let basicConfigJSON = """
    {
      "aestheticScore": 6,
      "batchCount": 1,
      "batchSize": 1,
      "causalInference": 0,
      "causalInferencePad": 0,
      "cfgZeroInitSteps": 0,
      "cfgZeroStar": false,
      "clipLText": null,
      "clipSkip": 1,
      "clipWeight": 1,
      "compressionArtifacts": "disabled",
      "compressionArtifactsQuality": 43.1,
      "controls": [],
      "cropLeft": 0,
      "cropTop": 0,
      "decodingTileHeight": 640,
      "decodingTileOverlap": 128,
      "decodingTileWidth": 640,
      "diffusionTileHeight": 1024,
      "diffusionTileOverlap": 128,
      "diffusionTileWidth": 1024,
      "faceRestoration": null,
      "fps": 5,
      "guidanceEmbed": 3.5,
      "guidanceScale": 1,
      "guidingFrameNoise": 0.02,
      "height": 1024,
      "hiresFix": false,
      "hiresFixHeight": 448,
      "hiresFixStrength": 0.7,
      "hiresFixWidth": 448,
      "id": 0,
      "imageGuidanceScale": 1.5,
      "imagePriorSteps": 5,
      "loras": [
        {
          "file": "zit_natalie_illustrated_lora_f16.ckpt",
          "mode": "all",
          "weight": 0.6
        }
      ],
      "maskBlur": 1.5,
      "maskBlurOutset": 0,
      "model": "z_image_turbo_1.0_q8p.ckpt",
      "motionScale": 127,
      "negativeAestheticScore": 2.5,
      "negativeOriginalImageHeight": 0,
      "negativeOriginalImageWidth": 0,
      "negativePromptForImagePrior": true,
      "numFrames": 14,
      "openClipGText": null,
      "originalImageHeight": 0,
      "originalImageWidth": 0,
      "preserveOriginalAfterInpaint": true,
      "refinerModel": null,
      "refinerStart": 0.85,
      "resolutionDependentShift": false,
      "sampler": 17,
      "seed": 2980573769,
      "seedMode": 2,
      "separateClipL": false,
      "separateOpenClipG": false,
      "separateT5": false,
      "sharpness": 0,
      "shift": 3,
      "speedUpWithGuidanceEmbed": true,
      "stage2Guidance": 1,
      "stage2Shift": 1,
      "stage2Steps": 10,
      "startFrameGuidance": 1,
      "steps": 8,
      "stochasticSamplingGamma": 0.3,
      "strength": 1,
      "t5TextEncoder": true,
      "targetImageHeight": 0,
      "targetImageWidth": 0,
      "teaCache": false,
      "teaCacheEnd": -1,
      "teaCacheMaxSkipSteps": 3,
      "teaCacheStart": 5,
      "teaCacheThreshold": 0.2,
      "tiledDecoding": false,
      "tiledDiffusion": false,
      "upscaler": null,
      "upscalerScaleFactor": 0,
      "width": 1024,
      "zeroNegativePrompt": false
    }
    """

    private static let minimalConfigJSON = """
    {
      "model": "sd_xl_base_1.0_f16.ckpt",
      "sampler": 0,
      "steps": 20,
      "guidanceScale": 7.5,
      "seed": 42,
      "seedMode": 0,
      "strength": 1,
      "batchCount": 1,
      "batchSize": 1,
      "width": 512,
      "height": 512,
      "loras": [],
      "controls": []
    }
    """

    private static let controlNetConfigJSON = """
    {
      "model": "sd_v1.5_f16.ckpt",
      "sampler": 11,
      "steps": 30,
      "guidanceScale": 7.5,
      "seed": 123456,
      "seedMode": 0,
      "strength": 1,
      "batchCount": 1,
      "batchSize": 1,
      "width": 512,
      "height": 768,
      "loras": [
        {"file": "style_lora.safetensors", "weight": 0.8, "mode": "all"},
        {"file": "detail_lora.safetensors", "weight": 0.4, "mode": "unet"}
      ],
      "controls": [
        {
          "file": "canny_v11p.safetensors",
          "weight": 1.0,
          "guidanceStart": 0.0,
          "guidanceEnd": 1.0,
          "controlImportance": "balanced"
        },
        {
          "file": "depth_v11f1p.safetensors",
          "weight": 0.6,
          "guidanceStart": 0.2,
          "guidanceEnd": 0.8,
          "controlImportance": "prompt"
        }
      ]
    }
    """
}
#endif
